...
public class UserStarredRepositoriesFetcher {
	...
	public static void main(String[] args) {
		...
		userRepoExtractor.setType(RepositoryType.Starred.name());
		...
		for (User user : users) {
			...
			while (true) {
				try {
					String url = user.getUrl() + Constants.starredRepoURLString;
					userRepoExtractor.setUserId(user.getId());
					String userRepoJsonString = pullObject.fetchJSONString(url,
